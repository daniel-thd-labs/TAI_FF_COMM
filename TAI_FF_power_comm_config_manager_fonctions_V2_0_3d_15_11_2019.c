...
#include "TAI_FF_power_comm_buffers_12_9_2017.h"
...
//Gestion du channel select par le Congifg[5]*********************************************************************************************
		...
		case 8:	if(Config_registers[8]&0x01)							//Vérifie l'état de controle register 8 pour le mode de relais "ON"/"OFF"
					MASTER_RELAIS =1;
				else
					MASTER_RELAIS =0;
		break;
...
/*******************************************************************
*******************************************************************
* Function:        unsigned char Get_System_Size(void)
*
* PreCondition:    None
*
* Input:           None
...
*
* Side Effects:    None
* Notes:           Determine la configuration du system TAI
*
*
*
*******************************************************************
...
{
unsigned char sizeTest,Masters_Status_Flag,maxchan;
unsigned char WaitCnt=10;
Masters_Status_Flag=0;
while(!SEKA_ONLINE_FLAG)
	{
	Delais(1000);
	if(WaitCnt)
		WaitCnt--;
...
for(sizeTest=0;sizeTest<SYSTEMSIZE;sizeTest++)
	{
	if(RxSekaFaderData[sizeTest]!= 0xffff)
		{
		maxchan=sizeTest;
		Masters_Status_Flag=1;
		}
	}

MAX_Core_Number=1;//(maxchan/32)+1;
...
//master_slave_n=0;
spi1_msg_grp=0;
//Push_Config_data_In_Queue(15,Config_registers[15],0);
//VCA_Bank_Location=Slave_Index;
//if(Config_registers[16]>=Config_registers[15])
//	Config_registers[16]=VCA_Bank_Location;


if(Masters_Status_Flag)
	return 1;
return 0;
}
...
//---------------------------------------------------------------
//---------------------------------------------------------------
//FONCTIONS:void rxMasters(unsigned char LocalConfigIndex,unsigned char LocalConfigData)
//Notes:
//Arguments:
//Variables locals:
//Variables globals:
//---------------------------------------------------------------
//---------------------------------------------------------------
void RxMasters(unsigned char LocalConfigIndex,unsigned char LocalConfigData)
{
