...
//PTEIA485 INIT
//------------------------------------------------------------------------
void PTEIA485StopInterrupts(void)
{
...
//PTEIA485 INIT
//------------------------------------------------------------------------
void PTEIA485StartInterrupts(void)
{
//no change in UART2 mode
IFS1bits.U2RXIF = 0;		//Rx interrupt flag clear
IEC1bits.U2RXIE = 1;		//Stop Rx interrupt enable bit and run in polled mode
}

//------------------------------------------------------------------------
//PTUART1 Stop Interrupts
//------------------------------------------------------------------------
...
switch (Command)
	...
	default:
		while(1)
			{
			while(passThroughRx485Flag!=2);//wait for EIA485 transmit to finish last message
				{
...
unsigned char getEIA232RxMsgLenght(void){return EIA232RxDataCount;}
unsigned char getEIA232TxMsgLenght(void){return EIA232TxDataCount;}
unsigned char getEIA485RxMsgLenght(void){return passThroughRx485Index; }
unsigned char getEIA485TxMsgLenght(void){return passThroughTx485Index;}
